 ...
 * downstream security rules (e.g. @PreAuthorize) work as expected.
 *
 * IMPORTANT: this service must only be reachable through the API Gateway.
 * Direct exposure to the internet without the gateway would bypass auth.
 */
public class GatewayHeaderAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (username != null && role != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            // Prefix role with ROLE_ if the gateway sends plain role names (e.g. "ADMIN")
